...
async def creat_Item(item : Annotated[InventoryItemBase, Query()]):
...
async def get_specific_item(itemname : str, itemid : Annotated[str | None, Query(max_length=10)] = None):
    result = await InventoryItemBase.find(InventoryItemBase.itemName == itemname or InventoryItemBase.itemID == itemid).to_list()
...
async def update_value(new_data : Annotated[InventoryItemBase, Query()]):
    old_item = await InventoryItemBase.get(new_data.id)
